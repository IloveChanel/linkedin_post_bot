...
class PostFetcher:  
    ...
    def __init__(self, use_local_fallback=True):
        ...
        Args:
            use_local_fallback: If True, will try local post/ folder if Google Drive fails
        """
        self.folder_id = os. getenv('GOOGLE_DRIVE_FOLDER_ID')
        self.credentials_json = os. getenv('GOOGLE_CREDENTIALS')
        self.rotation_file = Path(__file__).parent / 'posted.json'
        self.local_posts_dir = Path(__file__).parent.parent / 'post'  # ← Changed to 'post' (singular)!
        ...
        if self.credentials_json and self.folder_id:
            try:
                ...
                if use_local_fallback: 
                    logger.info("Will use local post folder as fallback")
    ...
    def get_next_post_from_local(self) -> Optional[str]:
        """Get the next post from local post/ folder"""
        try:
